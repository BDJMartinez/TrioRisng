...
namespace UndeadWarfare.Player
{
    ...
    public class PlayerController : MonoBehaviour, EnemyDamage, UndeadWarfare.AI.IDamage
    {
        ...
        // Take damage from IDamage interface
        public void TakeDamage(int amount, GameObject source = null, bool weakspot = false)
        {
          if (source == null) { source = GetComponent<GameObject>(); }
            health -= amount;
            Debug.Log($"The Player has been injured by {source}");
            updatePlayerUI();
            StartCoroutine(damageFlash());
        }
